...
RegistryManager::RecurseOpenKey(LPCTSTR lpszSection, REGSAM samDesired)
{
  ...
  if (hKeyBase == 0)
  {
	...
	while (nIndex != -1)
	{
		...
		m_lResult = ::RegOpenKey(hParentKey == nullptr ? hKeyBase : (HKEY)hParentKey,strKey,&hKey);
//                 ::RegCreateKeyEx(hParentKey == nullptr ? hKeyBase : (HKEY)hParentKey
//                                 ,strKey
//                                 ,0
//                                 ,REG_NONE
//                                 ,REG_OPTION_NON_VOLATILE
...
//                                 ,&dw);
  ...
  HKEY hAppKey = nullptr;
  // If we want to read, only open already existing keys
  if(samDesired == KEY_READ)
  {
    m_lResult = ::RegOpenKey(hParentKey,strRegistryKey,&hAppKey);
  }
  else
  {
